...
CHESSBOARD_SIZE = (6, 8)
SQUARE_SIZE = 17  # <- A4 printed chessboard, otherwise chessboard on my phone -> 7.25  # in mm
...
def rectifyImages(imL, imR, calib_data='calib_data/calibration.npz', cropped=False):
    ...
    # K1 = np.loadtxt("calib/CameraMatrixLeft.dat")
    # D1 = np.loadtxt("calib/DistortionCoefficientLeft.dat")
    # R1 = np.loadtxt("calib/R1.dat")
    # P1 = np.loadtxt("calib/P1.dat")
    #
    # leftMapX2, leftMapY2 = cv2.initUndistortRectifyMap(K1, D1, R1, P1, imageSize, cv2.CV_32FC1)
    # rectifiedL = cv2.remap(imL, leftMapX2, leftMapY2, cv2.INTER_LINEAR)
    #
    # K2 = np.loadtxt("calib/CameraMatrixRight.dat")
    # D2 = np.loadtxt("calib/DistortionCoefficientRight.dat")
    # R2 = np.loadtxt("calib/R2.dat")
    # P2 = np.loadtxt("calib/P2.dat")
    #
    # rightMapX2, rightMapY2 = cv2.initUndistortRectifyMap(K2, D2, R2, P2, imageSize, cv2.CV_32FC1)
    # rectifiedR = cv2.remap(imR, rightMapX2, rightMapY2, cv2.INTER_LINEAR)
...
def splitStereoImage(stereo_image):
    ...
    imgL = stereo_image[100:-100, 100:imgWidth, :]
    imgR = stereo_image[100:-100, imgWidth:-100, :]
    return imgL, imgR
